(function($){
    ...
    $.get(gotcha_client + '/api/applications/{{.Id.Hex}}/urls?token='+ token, function(response){
        ...
        $('.remove').on('click', function(element){
            ...
            $.ajax({
                url: gotcha_client + '/api/applications/{{.Id.Hex}}/urls/' + removeUrlId + '/?token=' + token,
                type: 'DELETE',
                complete: function(){
    ...
    $('#fetchUrls').on('click', function(){
        $.get(gotcha_client + '/api/applications/{{.Id.Hex}}/fetch?token='+ token, function(response){
            window.location.reload();
        });
    });
    ...
    $('.submit-url').on('click', function(){
        ...
        $.ajax({
            url: gotcha_client + '/api/applications/{{.Id.Hex}}/urls?token=' + token,
            type: "POST",
        ...
        $('.submit-callback').on('click', function(){
            ...
            $.ajax({
                url: gotcha_client + '/api/applications/{{.Id.Hex}}/addcallback?token=' + token,
                type: "POST",
                data: JSON.stringify(data),
                contentType: "application/json",
                complete: function(){
